...
GAMES_PER_REQUEST = 5  # Optimized for 75 games - perfect batch size!
TOTAL_GAMES = 75  # This will give us 3,750-7,500+ positions per master! 🚀
...
def request_complete_games(client: OpenAI, player_name: str) -> List[Dict[str, Any]]:
    ...
    for batch in range(batches):
        progress_pct = ((batch + 1) / batches) * 100
        print(f"📥 Batch {batch + 1}/{batches} ({progress_pct:.1f}%) - Requesting games...")
...
def main():
    ...
    try:
        ...
        games = request_complete_games(client, player_name)

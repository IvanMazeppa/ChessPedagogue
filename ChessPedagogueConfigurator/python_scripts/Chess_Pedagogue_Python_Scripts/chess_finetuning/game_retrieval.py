...
class NimzowitschTrainingPipeline:
    ...
    def fetch_nimzowitsch_games(self) -> List[ChessGame]:
        """Fetch famous Nimzowitsch games using Chat Completions with structured output"""
        self.create_emoji_log("🔍", "Fetching famous Nimzowitsch games...")
        
        # Using the new model that works well
        model = "gpt-4.1-2025-04-14"  # Your proven model
        
        # Structured prompt that matches your JSON schema exactly
        prompt = """You are a chess historian. Provide exactly 10 of Aron Nimzowitsch's most famous games in JSON format.

IMPORTANT: Include games that showcase:
- The Immortal Zugzwang Game vs Sämisch (Copenhagen 1923)
- Games against world champions (Capablanca, Alekhine)
- Prophylactic masterpieces
- Blockade demonstrations
- Hypermodern opening treatments

For EACH game, you MUST provide:
1. opponent: Full name (e.g., "Friedrich Sämisch")
2. year: 4-digit year (e.g., "1923")
3. tournament: Exact tournament name (e.g., "Copenhagen")
4. opening: Standard opening name (e.g., "Queen's Indian Defense")
5. moves: COMPLETE game in standard notation (e.g., "1. d4 Nf6 2. c4 e6...")
6. result: Must be "1-0", "0-1", or "1/2-1/2"
7. significance: Why this game matters

Provide the complete game moves - do NOT truncate!"""
        
        try:
            self.create_emoji_log("🎯", f"Using model: {model}")
            
            # Define the response format matching your schema
            response_format = {
                "type": "json_schema",
                "json_schema": {
                        ...
                        "properties": {
                            ...
                            "games": {
                                ...
                                "items": {
                                    ...
                                    "required": ["opponent", "year", "tournament", "opening", 
                                               "moves", "result", "significance", "white_player", "black_player"],
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system", 
                        "content": "You are a chess historian specializing in Aron Nimzowitsch. Always specify who played white and black in each game."
                    },
                    {"role": "user", "content": prompt}
                ],
                temperature=0.2,  # Lower temperature for more consistent results
                max_tokens=16000,  # Ensure we don't truncate
                response_format=response_format
            )
            
            games_data = json.loads(response.choices[0].message.content)
            self.create_emoji_log("📝", f"Received data for player: {games_data.get('player_name', 'Unknown')}")
            
            games = []
            for i, game_dict in enumerate(games_data.get("games", [])):
                try:
                    game = ChessGame(
                    ...
                    if valid:
                        games.append(game)
                        self.create_emoji_log("✅", f"Game {i+1}: {game.white_player} vs {game.black_player} ({game.year}) - Valid!")
                    else:
                        self.create_emoji_log("⚠️", f"Game {i+1}: {game.opponent} - Invalid PGN: {error}")
                        
                except KeyError as e:
                    self.create_emoji_log("❌", f"Game {i+1}: Missing field {e}")
                except Exception as e:
                    self.create_emoji_log("❌", f"Game {i+1}: Error - {str(e)}")
            
            self.create_emoji_log("📊", f"Successfully validated {len(games)} out of {len(games_data.get('games', []))} games")
            
            if len(games) < 5:
                self.create_emoji_log("⚠️", "Few games validated, adding fallback games...")
                games.extend(self.get_fallback_games())
            
            return games
            
        except Exception as e:
            self.create_emoji_log("❌", f"Error fetching games: {str(e)}")
            self.create_emoji_log("💡", "Using fallback games...")
